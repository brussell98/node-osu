...
class Api {
	...
	async getBeatmaps(options) {
		...
		return resp.map(bm => new Beatmap(bm));
	}
	...
	async getScores(options) {
		...
		if (!this.completeScores)
			return resp.map(sc => new Score(sc));

		const beatmaps = await this.getBeatmaps({ b: options.b });
		return resp.map(sc => new Score(sc, beatmaps[0]));
	}
	...
	async getUserBest(options) {
		...
		const scores = [];
		for (const score of resp) {
			const beatmaps = await this.getBeatmaps({ b: score.beatmapId });
			scores.push(new Score(score, beatmaps[0]));
		}
